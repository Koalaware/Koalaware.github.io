---
layout: post
title: CyberSpace CTF 2025
date: 2025-02-14 14:31 +0100
last_modified_at: 2025-02-14 14:31 +0100
categories: [ctf]
tag : [forensics, rev, mobile]
image: /assets/ctf/csctf2025/logo.png
---
...
<a href="/assets/zip/csctf2025/handout_social-distancing.zip" class="btn btn-primary" download>
...
i proceed by placing the quarantine folder inside my vms c drive in c:\programdata\microsoft\windows defender\ and run the script
...
- metadata: details such as the levels name, description, creator, and the level id
- player progress: the players progress in these custom levels, including completion status and scores
...
![alt text](../assets/ctf/csctf2025/cyberchef.png)
...
![alt text](../assets/ctf/csctf2025/gdbrowser1.png)

it seems to be a level made by user csctfa52de5, which was mentionned last in the xml file. checking the levels comments section reveals the flag

![alt text](../assets/ctf/csctf2025/gdbrowser2.png)
...
<a href="/assets/zip/csctf2025/handout_3d_obj.zip" class="btn btn-primary" download>
...
![alt text](../assets/ctf/csctf2025/3d0.png)

nothing seemed out of the ordinary, inspecting the inside of cube doesnt reveal anything, but zooming out glitches a bit and kind of reveals some weird looking squares

![alt text](../assets/ctf/csctf2025/3d0.5.png)

interesting, i export the cube in .glb format then head to threejss online editor. there, it becomes obvious by tinkering settings like blending

![alt text](../assets/ctf/csctf2025/3d1.png)
...
i start off by checking the memorys operating system profile, but it takes too long so i kind of cheat and use volatilitys windows cmdline plugin which confirms that it is indeed a dump of a windows machines memory
...
this reveals two important pieces of information, one, the hint provided in the description of the challenge matches process of pid 3212 responsible of note.txt in ggs desktop, two, the process of pid 1120 msdt.exe ran an obfuscated powershell payload
...
this reveals that a powershell script was run on ggs machine, which pushes me to dump the powershell evtx file in order to understand more
...
![alt text](../assets/ctf/csctf2025/memory1.png)
the content being:
...
now i use once again cyberchef to decrypt the flag.encs content
![alt text](../assets/ctf/csctf2025/memory2.png)

downloading the image reveals the flag
![alt text](../assets/ctf/csctf2025/memory3.png)
...
![alt text](../assets/ctf/csctf2025/pml3.png)
...
<a href="/assets/zip/csctf2025/chall.pcap.gz" download>
...
![alt text](../assets/ctf/csctf2025/packet0.png)
...
<a href="/assets/zip/csctf2025/login.apk" download>
...
![alt text](../assets/ctf/csctf2025/login0.png)
...
![alt text](../assets/ctf/csctf2025/login1.png)
...
![alt text](../assets/ctf/csctf2025/login2.png)
...
![alt text](../assets/ctf/csctf2025/login3.png)
...
<a href="/assets/zip/csctf2025/securenotes.apk" download>
...
![alt text](../assets/ctf/csctf2025/sn0.png)
...
this implies that we have to intercept the traffic in order to understand how the application work. at first, i tried to use [reflutter](https://github.com/ptswarm/reflutter) for traffic interception but it didnt work for my case setup due to unknown reasons even though i could capture https traffic on my burpsuite proxy. i was stuck for some time trying to make the setup work, but it didnt get any better until the organizers suggested me a tool i was unaware of, [http toolkit](https://httptoolkit.com/)

i begin by launching the toolkit on the emulator to connect it to the proxy on host machine

![alt text](../assets/ctf/csctf2025/sn3.png)

then, i try to signup and register a new user. which is successfuly captured on the view section

![alt text](../assets/ctf/csctf2025/sn4.png)

once were logged in, we have a bearer token and the app queries the notes of the user

![alt text](../assets/ctf/csctf2025/sn5.png)

it is empty for now

![alt text](../assets/ctf/csctf2025/sn6.png)

i create a note of appreciation to complete testing all endpoints

![alt text](../assets/ctf/csctf2025/sn7.png)
...
![alt text](../assets/ctf/csctf2025/sn8.png)
...
![alt text](../assets/ctf/csctf2025/sn9.png)
...
![alt text](../assets/ctf/csctf2025/sn10.png)
